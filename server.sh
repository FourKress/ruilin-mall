...
if pm2 ls | grep -q $app_name; then
  ...
  if [[ "$app_status" == *"online"* ]]; then
    ...
    MALL_APP_NAME=$app_name pm2 reload pm2.config.cjs --env $1
  elif [[ "$app_status" == *"stopped"* ]]; then
    echo "---应用停止运行，执行restart重新启动---"
    MALL_APP_NAME=$app_name pm2 restart pm2.config.cjs --env $1
  fi
else
  echo "---应用没有启动，执行start启动---"
  MALL_APP_NAME=$app_name pm2 start pm2.config.cjs --env $1
fi
